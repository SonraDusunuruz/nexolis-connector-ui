<script lang="ts">
	...
	import Connections from '../../../data/product.json';
...
<main class="relative h-full w-full overflow-y-auto bg-white dark:bg-gray-800">
	<div class="p-4">
		<Breadcrumb class="mb-5">
			<BreadcrumbItem home>Settings</BreadcrumbItem>
			<BreadcrumbItem>Connection Configurations</BreadcrumbItem>
		</Breadcrumb>
		...
		<Toolbar embedded class="w-full py-4 text-gray-500 dark:text-gray-400">
			<Input placeholder="Search for configuration" class="me-6 w-80 border xl:w-96" />
			<ToolbarButton
				color="dark"
	...
	<Table>
		<TableHead class="border-y border-gray-200 bg-gray-100 dark:border-gray-700">
			...
			{#each ['Name', 'PORT', 'Description', 'Protocol', 'Status', 'Actions'] as title}
		...
		<TableBody>
			{#each Connections as connection}
				<TableBodyRow class="text-base">
					...
					<TableBodyCell class="p-4">{connection.technology}</TableBodyCell>
					<TableBodyCell
						class="max-w-sm overflow-hidden truncate p-4 text-base font-normal text-gray-500 xl:max-w-xs dark:text-gray-400"
						>{connection.description}</TableBodyCell
					>
					<TableBodyCell class="p-4">{connection.id}</TableBodyCell>
					<TableBodyCell class="p-4">
						{#if connection.price === 'ACTIVE'}
							<span
								class="me-2 rounded bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-800
						dark:bg-green-900 dark:text-green-300">{connection.price}</span
							>
						{:else}
							<span
								class="me-2 rounded bg-red-100
								 px-2.5 py-0.5 text-xs font-medium
								 text-red-800 dark:bg-red-900
								 dark:text-red-300">{connection.price}</span
							>
						{/if}
					</TableBodyCell>
